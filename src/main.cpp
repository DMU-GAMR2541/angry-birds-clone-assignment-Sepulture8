...
int main() {
    ...
    sf::Font font;
    font.loadFromFile("../assets/fonts/angry-birds-1.ttf");
    ...
    pigText.setPosition(10.0f, 90.0f);
    ...
    levelCompleteText.setPosition(150.0f, 250.0f);
    ...
    std::vector<std::pair<float, float>> plankPositions = {
        {460.0f, 520.0f},  // left pillar tower 1
        {520.0f, 520.0f},  // right pillar tower 1
        {590.0f, 520.0f},  // left pillar tower 2
        {650.0f, 520.0f},  // right pillar tower 2
    };

    for (int i = 0; i < plankPositions.size(); i++) {
        plankPtr.push_back(std::make_shared<Plank>(world, plankPositions[i].first, plankPositions[i].second, 10.0f, 60.0f, "../assets/Ang_Birds/plank.jpg"));
    }
    ...
    while (window.isOpen()) {
        ...
        if (!birdPtr.empty() && abilityActivated)
        {
            ...
            if (!bird->isAbilityUsed())
            {
                ...
                //tried implementing blackbird ability - not successfull
        ...
        // --- UPDATE ---
        catapult.update();
        for (auto& plank : plankPtr) plank->Update();
        ...
        for (auto& plank : plankPtr) plank->draw(window);



        // --- GAME TIMER ---
        int elapsed = (int)gameTimer.getElapsedTime().asSeconds();
        timerText.setString("Time: " + std::to_string(elapsed) + "s");
        window.draw(timerText);

        // --- PIG COUNTER ---
        pigText.setString("Pig: " + std::to_string(pigPtr.size()));
        window.draw(pigText);
